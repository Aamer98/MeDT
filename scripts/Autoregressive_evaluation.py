...
from utils import set_seed, predict_state, sample, calculate_scores
...
# Evaluate policy network on state predictor network on evaluation set
def test_loop(policy_model, state_predictor, ret, data_loader):
    ...
    for x, y, r, t, SAPS, SAPS_constituents, traj_len  in data_loader:
        ...
        sampled_action = sample(policy_model, state, 1, temperature=1.0, sample=True, actions=None, 
                rtgs=torch.tensor(rtgs, dtype=torch.long).to(device).unsqueeze(0).unsqueeze(-1), 
                timesteps=t[:,:state.shape[1]].unsqueeze(-1), ucrtg=atgs, traj_len=traj_len) 
        ...
        for i_step in range(10): 
                ...
                state = predict_state(state_predictor, episode_states, 1, 
                ...
                sampled_action = sample(policy_model, episode_states, 1, temperature=1.0, sample=True, 
                                actions=torch.tensor(actions, dtype=torch.long).to(device).unsqueeze(1).unsqueeze(0),
                                rtgs=torch.tensor(rtgs, dtype=torch.long).to(device).unsqueeze(0).unsqueeze(-1),
                                timesteps=t[:,:episode_states.shape[1]].unsqueeze(-1),
                                ucrtg=atgs, traj_len=traj_len)
...
def main(args):
    ...
    mconf = GPTConfig(STATE_VOCAB_SIZE, block_size, 
            n_layer=sp_LAYERS, n_head=N_HEAD, n_embd=sp_EMB, model_type='state', max_timestep=CONTEXT_LENGTH)
    ...
    mconf = GPTConfig(ACTION_VOCAB_SIZE, block_size, 
            n_layer=pol_LAYERS, n_head=N_HEAD, n_embd=pol_EMB, model_type=args.model_type, max_timestep=CONTEXT_LENGTH)
    ...
    states = test_loop(policy_model, sp_model, args.rtg, test_loader)

    # Calculate SAPS2 on estimated states
    scores = calculate_scores(states)

    print('mean SAPS2 = {:.2f}+-{:.2f}'.format(np.mean(np.array(scores)[:,:,0]), np.std(np.array(scores)[:,:,0]) / np.sqrt(np.size(np.array(scores)[:,:,0]))))
    print('Cardivascular: {:.2f}+-{:.2f}'.format(np.mean(np.array(scores)[:,:,1]), np.std(np.array(scores)[:,:,1]) / np.sqrt(np.size(np.array(scores)[:,:,1]))))
    print('Respiratory: {:.2f}+-{:.2f}'.format(np.mean(np.array(scores)[:,:,2]),  np.std(np.array(scores)[:,:,2]) / np.sqrt(np.size(np.array(scores)[:,:,2]))))
    print('Neurological:{:.2f}+-{:.2f}'.format(np.mean(np.array(scores)[:,:,3]), np.std(np.array(scores)[:,:,3]) / np.sqrt(np.size(np.array(scores)[:,:,3]))))
    print('Renal: {:.2f}+-{:.2f}'.format(np.mean(np.array(scores)[:,:,4]), np.std(np.array(scores)[:,:,4]) / np.sqrt(np.size(np.array(scores)[:,:,4]))))
    print('Hepatic: {:.2f}+-{:.2f}'.format(np.mean(np.array(scores)[:,:,5]), np.std(np.array(scores)[:,:,5]) / np.sqrt(np.size(np.array(scores)[:,:,5]))))
    print('Haematologic: {:.2f}+-{:.2f}'.format(np.mean(np.array(scores)[:,:,6]), np.std(np.array(scores)[:,:,6]) / np.sqrt(np.size(np.array(scores)[:,:,6]))))
    print('Other: {:.2f}+-{:.2f}'.format(np.mean(np.array(scores)[:,:,7]), np.std(np.array(scores)[:,:,7]) / np.sqrt(np.size(np.array(scores)[:,:,7]))))
...
if __name__ == "__main__":
    ...
    parser.add_argument("--sp_dir", "-sp", default="SP_ID0_nLayer4_nHead8_nEmb64_LR0.0006_BS128_SEED0", help="")
    parser.add_argument("--policy_dir", "-p", default="MeDTv1_ID0_nLayer4_nHead8_nEmb64_LR0.0006_BS128_SEED0", help="") 
